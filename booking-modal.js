...
const BookingModal = (() => {
  ...
  async function _renderAuthGate(defaultTab) {
    ...
    body.innerHTML = `
      <div class="bm-inline-auth">
        ...
        <!-- Google — the real Google button is mounted into this slot by
             _mountGoogleButton(). The button below is only a fallback for when
             Google's script cannot load at all. -->
        <div id="bmGoogleSlot" class="bm-google-slot"></div>
        <button class="bm-google-btn" id="bmGoogleFallback" onclick="BookingModal._bmGoogleSignIn()" hidden>
        ...
        <div id="bmGoogleErr" class="bm-ia-error"></div>
  ...
  // Render a genuine Google Identity Services button inside the booking modal.
  ...
  function _mountGoogleButton() {
    ...
    const tick = () => {
      // The slot is replaced whenever the modal re-renders — stop if it is gone.
      if (!document.getElementById('bmGoogleSlot')) return;
      ...
      waited += STEP;
      if (waited < LIMIT) { setTimeout(tick, STEP); return; }

      // Google's script never arrived (blocked by an extension, offline, etc.).
